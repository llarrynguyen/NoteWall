package com.larrynguyen.notewall.utils.recyclerdiff;
...
import com.larrynguyen.notewall.HeadedRecyclerAdapter;
 ...
 * Note: It is based on {@link android.support.v7.util.DiffUtil}
 * and intended to work with the {@link com.larrynguyen.notewall.HeadedRecyclerAdapter}
 * <p>
 ...
 * BXDiffUtil on a background thread.
 * <p>
 * BXDiffUtil uses Eugene W. Myers's difference algorithm to calculate the minimal number of updates
 * to convert one list into another. Myers's algorithm does not handle items that are moved so
 * BXDiffUtil runs a second pass on the result to detect items that were moved.
 * <p>
...
public class HeadedRecyclerDiffUtil
{
	...
	// Myers' algorithm uses two lists as axis labels. In BXDiffUtil's implementation, `x` axis is
	 ...
	 *
	 * @param cb The callback that acts as a gateway to the backing list data
	 * @param detectMoves True if BXDiffUtil should try to detect moved items, false otherwise.
	 *
	 * @return A DiffResult that contains the information about the edit sequence to convert the
	 * old list into the new list.
	 */
	...
	private static Snake diffPartial(Callback cb, int startOld, int endOld,
		...
		throw new IllegalStateException("BXDiffUtil hit an unexpected case while trying to calculate"
				+ " the optimal path. Please make sure your data is not changing during the"
				+ " diff calculation.");
	}

	/**
	 * A Callback class used by BXDiffUtil while calculating the diff between two lists.
	 */
	public abstract static class Callback {
		...
		/**
		 * Called by the BXDiffUtil to decide whether two object represent the same Item.
		 * <p>
		...
		/**
		 * Called by the BXDiffUtil when it wants to check whether two items have the same data.
		 * BXDiffUtil uses this information to detect if the contents of an item has changed.
		 * <p>
		 * BXDiffUtil uses this method to check equality instead of {@link Object#equals(Object)}
		 * so that you can change its behavior depending on your UI.
		 * For example, if you are using BXDiffUtil with a
		 ...
		 * {@link #areContentsTheSame(int, int)} returns false for them, BXDiffUtil
		 * calls this method to get a payload about the change.
		 * <p>
		 * For example, if you are using BXDiffUtil with {@link RecyclerView}, you can return the
	 ...
	 * {@link RecyclerView.Adapter} via {@link #dispatchUpdatesTo(HeadedRecyclerAdapter)}.
	 */
	public static class DiffResult {
		 ...
		 * <p>
		 * BXDiffUtil does this pre-processing so that if it is running on a big list, it can be moved
		 ...
		 *     DiffResult result = BXDiffUtil.calculateDiff(new MyCallback(oldList, newList));
